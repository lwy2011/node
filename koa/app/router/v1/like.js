...
const router1 = new Router({prefix: "/v1/book"});


router.post("/like", new Auth(2).token, async (ctx) => {
    //前端传的数据：第一步，参数验证：type,art_id
    const v = await new LikeValidator("body", "likeClassicValidator").validate(ctx, {id: "art_id"});
    await Favor.like(
        v.get("body.art_id"),
        v.get("body.type"),
...
router1.post("/like", new Auth(2).token, async (ctx) => {
    //前端传的数据：第一步，参数验证：type,art_id
    const v = await new LikeValidator("body", "likeClassicValidator").validate(ctx, {id: "art_id"});
    await Favor.like(
        v.get("body.art_id"),
...
router1.post("/dislike", new Auth(2).token, async (ctx) => {
    //前端传的数据：第一步，参数验证：type,art_id
    const v = await new LikeValidator("body", "likeClassicValidator").validate(ctx, {id: "art_id"});
    await Favor.dislike(
        v.get("body.art_id"),
